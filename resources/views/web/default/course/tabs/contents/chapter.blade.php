                ...
                @if((!empty($chapter->chapterItems) and count($chapter->chapterItems)) or (!empty($chapter->quizzes) and count($chapter->quizzes)))
                    <div class="accordion-row rounded-sm border mt-20 p-15">
                        <div class="d-flex align-items-center justify-content-between" role="tab" id="chapter_{{ $chapter->id }}">
                            <div class="js-chapter-collapse-toggle d-flex align-items-center collapse" href="#collapseChapter{{ $chapter->id }}" aria-controls="collapseChapter{{ $chapter->id }}" data-parent="#chaptersAccordion" role="button" data-toggle="collapse" aria-expanded="true">
                                <span class="chapter-icon mr-15">
                                    <i data-feather="grid" class=""></i>
                                </span>

                                <span class="font-weight-bold text-secondary font-28" style="font-size: 30px!important;">{{ $chapter->code }}-{{ $chapter->title }}</span>
                            </div>
                            ...
                            <div class="d-flex align-items-center">
                                ...
                                <i id="cls" class="collapse-chevron-icon" data-feather="chevron-down" height="20" href="#collapseChapter{{ !empty($chapter) ? $chapter->id :'record' }}" aria-controls="collapseChapter{{ !empty($chapter) ? $chapter->id :'record' }}" data-parent="#chaptersAccordion" role="button" data-toggle="collapse" aria-expanded="true"></i>
                            </div>
                        </div>
                                ...
                                @if(!empty($chapter->chapterItems) and count($chapter->chapterItems))
                                    ...
                                    @foreach(collect($chapter->chapterItems)->sortByDesc('type') as $chapterItem)
                                        ...
                                        @elseif($chapterItem->type == \App\Models\WebinarChapterItem::$chapterFile and !empty($chapterItem->file) and $chapterItem->file->status == 'active')
                                            ...
                                            @if($chapterItem->file->upload_file_type == 1)
                                                @if(!$hasClassroomLearningGuide)
                                                    <br>
                                                    <h5>Classroom Learning Guide</h5>
                                                    @php $hasClassroomLearningGuide = true; @endphp
                                                @endif
                                                @include('web.default.course.tabs.contents.files' , ['file' => $chapterItem->file, 'accordionParent' => 'chaptersAccordion'])
                                                
                                            @elseif($chapterItem->file->upload_file_type == 2)
                                                @if(!$hasStudentHandbook)
                                                    <br>
                                                    <h5>Student Handbook</h5>
                                                    @php $hasStudentHandbook = true; @endphp
                                                @endif
                                                @include('web.default.course.tabs.contents.files' , ['file' => $chapterItem->file, 'accordionParent' => 'chaptersAccordion'])
                                            @elseif($chapterItem->file->upload_file_type == 3)
                                                @if(!$hasAssessment)
                                                    <br>
                                                    <h5>Trainer Assessment Guide</h5>
                                                    @php $hasAssessment = true; @endphp
                                                @endif
                                            @endif
