...
class AssignmentController extends Controller
{
    ...
    public function myCoursesAssignments(Request $request)
    {
        ...
        $assignmentsCollection = Collection::make($assignments->items())->map(function ($assignment) use ($user) {
            $grades = $assignment->instructorAssignmentHistories->filter(function ($item) {
                return !is_null($item->grade);
            });
        ...
        $assignments = $assignments->setCollection($assignmentsCollection->sortByDesc('pendingCount')->values());
