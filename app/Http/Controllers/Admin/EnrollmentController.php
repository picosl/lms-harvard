...
class EnrollmentController extends Controller
{
    ...
    public function addStudentToClass()
    {
        ...
        $students = User::select('id', 'full_name as name')->where('role_name', 'student')->get();
    ...
    public function getUnits($course_id){
         ...
         $units=WebinarChapter::where('webinar_id',$course_id)->get();
    ...
    public function store(Request $request)
    {
        ...
        if (isset($units) &&  count($units) > 0) {
            ...
            $unit              = new EntakeUnit();
            $unit->student_id =$data['user_id'];
            $unit->webinar_id  =$data['webinar_id'];
            $unit->batch_id  =$data['batch_id'];
            $unit->unit_code    =$value['code'];
            $unit->unit_id    =$value['id'];
            
            $unit->unit_title    = $value['title'];
            $unit->durration        = $value['days'];
            $unit->start_date  = $value['start_date'];
            $unit->end_date  = $value['end_date'];  
            $unit->save();
        }
